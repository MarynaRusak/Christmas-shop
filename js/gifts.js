document.addEventListener("DOMContentLoaded", () => {
    ...
    // Helper function to get random unique items from the array
    function getRandomUniqueItems(arr, count) {
      const shuffled = [...arr].sort(() => 0.5 - Math.random());
      return shuffled.slice(0, count);
    }
  
    // Generate random 4 items
    const randomCards = getRandomUniqueItems(data, 4);
  ...
  container.addEventListener("click", (event) => {
      ...
      if (cardElement) {
          ...
          const selectedGift = randomCards[index];
