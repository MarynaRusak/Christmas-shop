document.addEventListener("DOMContentLoaded", () => {
  ...
  const shuffledData = shuffleArray(data);
    const categoryToImage = {
      "For Work": "img/gift-for-work.png",
      "For Health": "img/gift-for-health.png",
      "For Harmony": "img/gift-for-harmony.png",
    };
  
    const container = document.querySelector(".best-gifts-container");
    ...
    function renderCards(filteredData) {
      ...
      filteredData.forEach((card, index) => {
          const cardHTML = `
              <div class="btn-card" data-index="${index}">
                  <div class="card">
                      <div class="img-container">
                          <img src="${categoryToImage[card.category]}" alt="gift for ${card.category.toLowerCase()}">
                      </div>
                      <div class="card-text">
                          <p class="tag tag-${card.category.toLowerCase().replace("for ", "").replace(" ", "-")}">${card.category}</p>
                          <h3>${card.name}</h3>
                      </div>
                  </div>
              </div>
    ...
    tabLinks.forEach((tab) => {
        tab.addEventListener("click", (e) => {
            ...
            const category = tab.textContent.trim(); // Get tab text
            if (category === "All") {
                renderCards(shuffledData); // Show all cards
            } else {
                const filteredData = shuffledData.filter((card) => card.category.toLowerCase().includes(category.toLowerCase()));
    ...
    const modalLive = document.querySelector(".live");
    const modalCreate = document.querySelector(".create");
    const modalLove = document.querySelector(".love");
    const modalDream = document.querySelector(".dream");
    const modalTag = document.querySelector(".tag");

    // Function to open modal with the selected gift card details
    function openModal(gift) {
        modalImg.src = categoryToImage[gift.category];
        modalTitle.textContent = gift.name;
        modalDescription.textContent = gift.description;
        modalLive.textContent = gift.superpowers.live;
        modalCreate.textContent = gift.superpowers.create;
        modalLove.textContent = gift.superpowers.love;
        modalDream.textContent = gift.superpowers.dream;
 ...
 // Event listener for closing modal using close button
 modalCloseBtn.addEventListener("click", closeModal);

 // Event delegation to handle card click
 container.addEventListener("click", (event) => {
     // Check if a card was clicked
     const cardElement = event.target.closest(".btn-card");
     if (cardElement) {
         const index = cardElement.getAttribute("data-index");
         const selectedGift = shuffledData[index];
...
document.addEventListener("DOMContentLoaded", () => {
  const scrollButton = document.querySelector(".btn-arrow.top");
  ...
  window.addEventListener("scroll", () => {
      if (isSmallScreen() && window.scrollY > 300) {
          scrollButton.style.display = "flex"; // Show button for small screens
      } else {
          scrollButton.style.display = "none"; // Hide button otherwise
      }
  });
  ...
  // Handle window resize to ensure button behavior adjusts dynamically
  window.addEventListener("resize", () => {
      if (!isSmallScreen()) {
          scrollButton.style.display = "none"; // Ensure the button stays hidden on larger screens
      }
  });
});
